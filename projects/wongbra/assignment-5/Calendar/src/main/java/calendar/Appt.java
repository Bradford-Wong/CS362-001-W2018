...
public class Appt implements  Comparable<Appt>{
    ...
    private void isValid() {
    	...
    	if(startHour<0 || startHour>23) //changed start hour to 14 --assignment 5, changed back to 0
    		this.valid=false;
    	else
    ...
    public void setTitle(String title) {
        if (title == null) 
        	...
        	this.title = title; //switched line 165 with line 167, switched back to original in assignment 5
    }
    ...
    private String represntationApp(){
        ...
        if (printableHour > 11)
        {
            printableHour -= 12; //changed 12 to 0, changed back to 12 in assignment 5
        }
        if (printableHour == 0)
        {
            printableHour = 12;
    ...
    public String toString()
    {
		if (!getValid()) {
         ...
         String day= this.getStartMonth()+"/"+this.getStartDay()+"/"+this.getStartYear() + " at "; //switched order of day and month, changed back to normal in assignment 5
